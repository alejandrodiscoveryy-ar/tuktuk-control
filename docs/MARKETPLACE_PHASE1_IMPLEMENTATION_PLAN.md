...
| 6. Atomic acceptance | RPC acepta `trial_free` o `wallet_commission`, reserva 10 % solo fuera de trial y liquida según modo congelado. | 4–5; carreras, reintentos, cancelación e incidente. | Cero doble reserva/cargo; rollback bloquea aceptación. |
...
| 12. Tests | Suites unitarias, RPC/RLS, frontera de 30 días y congelación de `billing_mode`. | Todos los anteriores. | Cobertura de invariantes y `git diff --check`; rollback no aplica. |
